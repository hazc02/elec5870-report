...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[x=1.5cm,y=0.6cm,font=\footnotesize]
    ...
    % S1805 with narrower opening (split to avoid outlines across the gap)
    ...
    \node[anchor=east] at (-0.2,\hsub+\hlor+0.5*\hs1803) {S1805};

    % Metal deposition: recessed metal aligned with S1805 edges
    \fill[gray!60] (\xL,\hsub) rectangle (\xR,\hsub+0.65);
    \draw (\xL,\hsub) rectangle (\xR,\hsub+0.65);
    \node at (3.5,\hsub+0.32) {AuGe/Ni/Au};

    % Metal atop resist (separate pieces to show broken continuity)
    \def\mtop{0.2}
    \fill[gray!60] (0.3,\hsub+\hlor+\hs1803) rectangle (\xL,\hsub+\hlor+\hs1803+\mtop);
    \draw (0.3,\hsub+\hlor+\hs1803) rectangle (\xL,\hsub+\hlor+\hs1803+\mtop);
    \fill[gray!60] (\xR,\hsub+\hlor+\hs1803) rectangle (\w-0.3,\hsub+\hlor+\hs1803+\mtop);
    \draw (\xR,\hsub+\hlor+\hs1803) rectangle (\w-0.3,\hsub+\hlor+\hs1803+\mtop);

  \end{tikzpicture}

  \vspace{5mm}

  \caption{Dual-layer LOR/S1805 resist stack used to form a lift-off undercut.}
