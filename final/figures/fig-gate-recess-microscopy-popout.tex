% ------------------------------------------------------------
% File: figures/fig-gate-recess-microscopy-popout.tex
% Gate recess microscopy callout: x20 image -> x100 image
% ------------------------------------------------------------

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
    callout/.style={dashed, line width=1.3pt},
    connector/.style={dashed, line width=0.8pt, opacity=0.65}
  ]
    % Manual adjustment controls:
    % - Source box on the x20 image. Fractions are measured from the top-left
    %   of the x20 image. Move these to place the box over the gate.
    \def\xTwentyBoxLeft{0.38}
    \def\xTwentyBoxRight{0.48}
    \def\xTwentyBoxTop{0.35}
    \def\xTwentyBoxBottom{0.49}

    \node[anchor=north west, inner sep=0pt] (xTwenty) at (0,0) {
      \includegraphics[width=0.48\linewidth, angle=180, origin=c]{figures/fabrication_images/enhancement_mode_images/GateRecess.jpg}
    };

    \node[anchor=north west, inner sep=0pt] (xHundred) at ($(xTwenty.north east)+(0.04\linewidth,0)$) {
      \includegraphics[width=0.48\linewidth]{figures/fabrication_images/enhancement_mode_images/GateRecess_100.jpg}
    };
  ...
  \vspace{3pt}
  \caption{Gate Recess Etch Trench Microscopy (20x Zoom Left, 100x Zoom Right)}
  \label{fig:gate-recess-trench}
\end{figure}
