...
Ensuring efficient use of cleanroom time; two quadrants, across the two samples were processed 
in parallel at the same time, requiring four repeated cycles of recess lithography, recess 
etching, gate lithography, and gate deposition to complete the full device set across eight quadrants.
...
\input{figures/fig-iteration-1-mask-popout.tex}
...
\begin{figure}[htbp]
  \centering
  \makebox[\linewidth][c]{\hspace*{-0.04\linewidth}\includegraphics[width=0.9\linewidth]{figures/electrical_analysis/enhancement_mode/iteration_1/65nm_gate_transfer.png}}
  \makebox[\linewidth][c]{\hspace*{-0.04\linewidth}\includegraphics[width=0.9\linewidth]{figures/electrical_analysis/enhancement_mode/iteration_1/65nm_output.png}}
  \vspace{3pt}
...
\begin{figure}[htbp]
  \centering
  \makebox[\linewidth][c]{\hspace*{-0.04\linewidth}\includegraphics[width=0.9\linewidth]{figures/electrical_analysis/enhancement_mode/iteration_1/53nm_65nm_gate_transfer.png}}
...
they did not span the full active conduction region controlled by the gate. As
a result, portions of the source--drain path remained unrecessed.

In these unrecessed regions, the original GaAs/AlGaAs heterostructure was left
intact. A 2DEG could therefore still form at zero gate
bias, as in the baseline depletion-mode devices. Current was able to flow
around the locally recessed trench through these adjacent unrecessed regions.
Consequently, even if the material directly beneath the recess slit was
depleted, over-etched, or otherwise electrically degraded, it did not fully
determine the measured drain current.
...
\input{figures/fig-current-path-bypass.tex}

This bypass path explains why the devices remained depletion-mode despite the
large measured recess depths. The gate-recess process modified only part of
the available current path, while the remaining unrecessed regions provided a
parallel channel that preserved normally-on conduction. The extracted
threshold voltage therefore reflects the combined response of the recessed and
unrecessed geometry, rather than the threshold of a uniformly recessed
channel. For this reason, the Iteration 1 data cannot be used to extract a
reliable relationship between recess depth and threshold voltage.
